...
CREATE DATABASE /*!32312 IF NOT EXISTS*/ b14_grabbuddy /*!40100 DEFAULT CHARACTER SET utf8mb4 */;
USE b14_grabbuddy;

DROP TABLE IF EXISTS accounts;
CREATE TABLE `accounts` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary Key',
  `owner_id` int(11) DEFAULT NULL COMMENT 'owner id',
  `type` varchar(30) DEFAULT NULL COMMENT 'wallet type',
  `wallet_limit` float DEFAULT '0',
  `wallet_balance` float DEFAULT '0',
  `restricted_transaction` varchar(50) DEFAULT 'none',
  `allowed_transaction` varchar(50) DEFAULT 'all',
  PRIMARY KEY (`id`),
  KEY `owner_id` (`owner_id`),
  KEY `restricted_transaction` (`restricted_transaction`),
  CONSTRAINT `account_fk_1` FOREIGN KEY (`owner_id`) REFERENCES `users` (`id`),
  CONSTRAINT `account_restrict` FOREIGN KEY (`restricted_transaction`) REFERENCES `transaction_categories` (`category`)
...
CREATE TABLE `quest` (
  ...
  `quest_description` varchar(255) DEFAULT 'Task',
  `transaction_value` int(10) unsigned DEFAULT NULL,
...
CREATE TABLE `transactions` (
  ...
  `category` varchar(50) DEFAULT NULL COMMENT 'transaction category',
  `transaction_amount` float DEFAULT '0',
  `payer_id` int(11) DEFAULT NULL COMMENT 'payer',
  `payee_id` int(11) DEFAULT NULL COMMENT 'payee',
  PRIMARY KEY (`id`),
  KEY `payer_id` (`payer_id`),
  KEY `payee_id` (`payee_id`),
  KEY `category` (`category`),
  CONSTRAINT `transaction_cats` FOREIGN KEY (`category`) REFERENCES `transaction_categories` (`category`),
  CONSTRAINT `transaction_payee` FOREIGN KEY (`payee_id`) REFERENCES `accounts` (`id`),
  CONSTRAINT `transaction_payer` FOREIGN KEY (`payer_id`) REFERENCES `accounts` (`id`),
  CONSTRAINT `transactions_ibfk_1` FOREIGN KEY (`category`) REFERENCES `transaction_categories` (`category`),
  CONSTRAINT `transactions_ibfk_2` FOREIGN KEY (`category`) REFERENCES `transaction_categories` (`category`)
) ENGINE=InnoDB AUTO_INCREMENT=36 DEFAULT CHARSET=utf8mb4;

DROP TABLE IF EXISTS transaction_categories;
CREATE TABLE `transaction_categories` (
  `category` varchar(50) NOT NULL COMMENT 'Primary Key',
  `comment` text COMMENT 'transaction type',
...
CREATE TABLE `users` (
  ...
  `mobile` int(10) unsigned DEFAULT NULL COMMENT 'mobile phone number',
  `email` varchar(255) DEFAULT NULL COMMENT 'Email',
  `login_id` varchar(255) DEFAULT NULL COMMENT 'Login ID',
  `login_password` varchar(255) DEFAULT NULL COMMENT 'Password',
  `user_type` varchar(205) DEFAULT NULL COMMENT 'UserType',
  `parent_id` int(10) unsigned DEFAULT NULL COMMENT 'foreign key- null for parents, otherwise points to childs parent id',
  PRIMARY KEY (`id`)
...
CREATE PROCEDURE `CheckRestriction`(
	IN userid INT, category VARCHAR(255)
	)
BEGIN
	DECLARE restricted VARCHAR(50) DEFAULT NULL;
	SELECT *
	FROM accounts 
